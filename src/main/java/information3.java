...
public class information3 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String useracademy=(String)session.getAttribute("useracademy");
		System.out.println(page);
		System.out.println("ua"+useracademy);
		if(flag==null)
		...
		if(flag.equals("2"))
			...
			try {
				...
				String sql="select distinct subject from acsucl where academy=?";
				if(search!="")
					sql+=" and(subject=?)";
				PreparedStatement pstmt=dbconn.prepareStatement(sql);
				pstmt.setString(1,useracademy);
				if(search!="")
				{
		...
		else if(flag.equals("4"))
			{
				...
				try {
					...
					dataSource=(DataSource)context.lookup("java:comp/env/jdbc/connect");
					 dbconn=dataSource.getConnection();
					String sql="select * from teacher where academy=?";
					if(search!="")
						sql+=" and( name=? or id=? or academy=? or jobnumber=? or role=?)";
					PreparedStatement pstmt=dbconn.prepareStatement(sql);
					pstmt.setString(1,useracademy);
					if(search!="")
					{					
			...
			try {
				...
				dataSource=(DataSource)context.lookup("java:comp/env/jdbc/connect");
				dbconn=dataSource.getConnection();
				String sql="select * from student where academy=?";
				if(search!="")
					sql+=" and( name=? or id=? or academy=? or studentnumber=? or subject=? or class=?)";
				PreparedStatement pstmt=dbconn.prepareStatement(sql);
				pstmt.setString(1,useracademy);
				if(search!="")
				{
		...
		getServletConfig().getServletContext().getRequestDispatcher("/subject.jsp").forward(request, response);
